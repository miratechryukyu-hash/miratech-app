...
def check_auth():
    ...
    fid = query_params.get("fid", "")     # 施設コード
    token = query_params.get("key", "")   # 秘密鍵
...
with tabs[3]:
    ...
    base_url = st.text_input("このアプリのURL（ブラウザの上のアドレス）", value="ここに新しいアプリのURLを貼り付けてください")
    target_qr_me = st.text_input("🔤 QRコードを作りたい「ME No.」を入力", placeholder="例: TE-381-001")
    
    if st.button("QRコードを作成する"):
        if base_url and target_qr_me and auto_fid and auto_token:
            # ✨パラメータに fid と key を追加！
            if base_url.endswith("/"):
                final_url = f"{base_url}?fid={auto_fid}&key={auto_token}&me_no={target_qr_me}"
            else:
                final_url = f"{base_url}/?fid={auto_fid}&key={auto_token}&me_no={target_qr_me}"
            ...
            st.warning("アプリのURLと、ME No.の両方を入力してください。（ログイン状態も確認してください）")
